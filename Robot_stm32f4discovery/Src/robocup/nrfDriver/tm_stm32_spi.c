...
/* Private functions */
static void TM_SPIx_Init(SPI_TypeDef* SPIx, TM_SPI_Mode_t SPI_Mode,
		uint16_t SPI_BaudRatePrescaler, uint16_t SPI_MasterSlave,
...
#ifdef SPI1
	if (SPIx == SPI1) {
		TM_SPIx_Init(SPI1, TM_SPI1_MODE, TM_SPI1_PRESCALER, TM_SPI1_MASTERSLAVE,
				TM_SPI1_FIRSTBIT);
	}
#endif
#ifdef SPI2
	if (SPIx == SPI2) {
		TM_SPIx_Init(SPI2, TM_SPI2_MODE, TM_SPI2_PRESCALER, TM_SPI2_MASTERSLAVE,
				TM_SPI2_FIRSTBIT);
	}
#endif
}
...
void TM_SPI2_INT_InitPins() {
	...
	TM_GPIO_InitAlternate(GPIOB, GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15,
			TM_GPIO_OType_PP, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_High,
			GPIO_AFx_SPI2);
	TM_GPIO_InitAlternate(GPIOC, GPIO_PIN_2, TM_GPIO_OType_PP,
			TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_High, GPIO_AFx_SPI2);
}
#endif
